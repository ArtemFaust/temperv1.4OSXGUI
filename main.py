...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.thread.emiter.connect(self.EmiterPrint)  # Подключаем сигнал из потока к локальной функции
...
# Минутный график
    def plot(self):
        ...
        if len(sp) >= 60:
            ...
            try:
                self.figure_graph1_ax.plot([int(self.lineEdit_4.text()) for x in range(0, 60, 1)],color='red',label='Critical')
            except:
                pass
        else:
            self.figure_graph1_ax.plot(sp,color='green',label='Current')
            try:
                self.figure_graph1_ax.plot([int(self.lineEdit_4.text()) for x in range(0, len(sp), 1)], color='red', label='Critical')
            except:
                pass
        self.figure_graph1_ax.set_title("Минутная статистика")
    ...
    def EmiterPrint(self, emit_text):
        ...
        if emit_text != "Non connection!" and emit_text != "Connection lost!":
            try:
                ...
                if int(float(emit_text) + float(self.lineEdit_5.text())) > int(self.lineEdit_4.text()):
                        ...
                        if os.path.exists('conf.ini') and not sendmail_status:
                            ...
                            try:
                                ...
                                body = "Temperature allert! = " + str(
                                    int(float(emit_text) + float(self.lineEdit_5.text())))
                ...
                self.label_13.setText(str(float(emit_text) + float(self.lineEdit_5.text())) + " C˚")
                if int(float(max(sp)) + float(self.lineEdit_5.text())) > int(self.lineEdit_4.text()):
                    self.label_16.setStyleSheet('color: red')
                else:
                    self.label_16.setStyleSheet('color: green')
                self.label_16.setText(str(max(sp)) + " C˚")
                if int(float(min(sp)) + float(self.lineEdit_5.text())) > int(self.lineEdit_4.text()):
...
class ThreadToReadTemper(QtCore.QThread):
    ...
    emiter = QtCore.pyqtSignal(str)  # эмитер (сигнал) который передает переменную тима строки
        ...
        while True:
            ...
            try:
                ...
                if int(temp_c) == 77 or int(temp_c) == 208:
                    ...
                    self.emiter.emit(str(float("%.3f" % temp_c)))  # вызывает функцию emit у эмитера для передачи сигнала в основной поток
                time.sleep(1)
                h.close()
                open = False
            except:
        ...
        if self.lineEdit.text() and self.lineEdit_3.text():
            if "@gmail.com" in self.lineEdit.text():
                if self.textEdit.toPlainText() != "":
                    ...
                    file.write("Sendler=%s" % self.lineEdit.text() +
                    ...
                    QtWidgets.QMessageBox.information(self, 'Уведомление',
                                                      'Не указанны получатели!')
            else:
                QtWidgets.QMessageBox.information(self,'Уведомление','Можно использовать только\nпочтовые ячики gmail!')
